...
`CURRENT.md` and `CURRENT.candidate.md` are optional derived resume projections stored beside the canonical artifacts in the same work directory, never contract, strategy, execution evidence, or report input. They may be replaced or deleted at any time; absence restores the full SPEC + PLAN + TRACK path. `references/context.md` is their only normative protocol.
